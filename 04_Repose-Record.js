// --- Day 4: Repose Record ---
// You've sneaked into another supply closet - this time, it's across from the prototype suit manufacturing lab. You need to sneak
// inside and fix the issues with the suit, but there's a guard stationed outside the lab, so this is as close as you can safely get.
...
// 11-04  #99  ....................................##########..............
// 11-05  #99  .............................................##########.....
// The columns are Date, which shows the month-day portion of the relevant day; ID, which shows the guard on duty that day; and
// Minute, which shows the minutes during which the guard was asleep within the midnight hour. (The Minute column's header shows
// 	the minute's ten's digit in the first row and the one's digit in the second row.) Awake is shown as ., and asleep is shown as #.

// Note that guards count as asleep on the minute they fall asleep, and they count as awake on the minute they wake up. For example,
...
// In the example above, Guard #10 spent the most minutes asleep, a total of 50 minutes (20+25+5), while Guard #99 only slept for a
// total of 30 minutes (10+10+10). Guard #10 was asleep most during minute 24 (on two days, whereas any other minute the guard was
...
const input = require('./04_input.js');
...
Guard object:
	...
	hour: HH (number)
	mins: MM (number)
	activity:  '#0000' or 'up' or 'asleep'
	timestamp:  (date * 24 * 60) + (hour * 60) + mins)
...
input.forEach((record,i) => {
	if (record.activity[0] === '#') {
		if (!guards.hasOwnProperty(record.activity)) {
			guards[record.activity] = {
				log: new Array(60),
				total: 0
			};
			guards[record.activity].log.fill(0);
		}
		currentGuard = record.activity;
	} else if (record.activity === 'up') {
		let asleep = input[i-1].mins;
...
console.log(sleepiestGuardNumber);
let heaviestSleep = Math.max(... guards[sleepiestGuardNumber].log);
console.log(heaviestSleep);
let sleepiestMinute = guards[sleepiestGuardNumber].log.indexOf(heaviestSleep);
