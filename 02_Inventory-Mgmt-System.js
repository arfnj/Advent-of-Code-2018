...
// "Wouldn't they have had enough fabric to fill several boxes in the warehouse? They'd be stored together, so the box IDs should
// be similar. Too bad it would take forever to search the warehouse for two similar box IDs..." They walk too far away to hear
// any more.
...
const input = require ('./02_input.js');
...
input.forEach(boxId => {
	let letterIndex = {};
	let letters = boxId.split('');
	letters.forEach(letter => {
		letterIndex[letter] = letterIndex[letter] + 1 || 1;
	})
	if (Object.values(letterIndex).includes(2)) {
		twos ++;
	}
	if (Object.values(letterIndex).includes(3)) {
		threes ++;
	}
});
...
for (let j=0; j<input.length; j++) {
	for (let i=0; i<input[j].length; i++) {
		let front = input[j].substring(0,i);
		let back = input[j].substring(i+1,input[j].length);
		let index = `-${i}`;
		let boxIdString = front + back + index;
		if (uniques.has(boxIdString)) {
			console.log(front + back);
			matchFound = true;
			break;
		}
		uniques.add(boxIdString);
	}
	if (matchFound) {
		break;
	}
