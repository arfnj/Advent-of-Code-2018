const raw = `[1518-03-11 00:45] wakes up
...
const clean = raw.split('\n').map(entry => {
	let dataObj = {}
	dataObj.date = Number(entry.slice(6,8) + entry.slice(9,11));
	dataObj.hour = Number(entry.slice(12,14));
	dataObj.mins = Number(entry.slice(15,17));
	dataObj.activity = entry.slice(19).split(' ')[1];
	dataObj.timestamp = (dataObj.date * 24 * 60) + (dataObj.hour * 60) + dataObj.mins;
	return dataObj;
});
